...
public boolean tomoElRamo(String sigla){
		...
		for (int i = 0; i<this.cargaAcademica.semestres.size() ; i++){
			...
			for (int j = 0; j<s.cursos.size() ; j++){
				...
				Curso c = (Curso) s.cursos.get(j);
				if (sigla.toLowerCase().equals(c.getSigla().toLowerCase())){
					return true;
				}
			}
		}
	...
	public boolean puedeTomarElRamo(String sigla){  
		if (sigla == "null")
			return true;
		Curso c = MainApp.verificador.retornarCursoSegunSigla(sigla);
		String Prerequisito = c.prerequisito ;
		
		if (Prerequisito.equals("null") || this.tomoElRamo(Prerequisito)){
			return true;
		}
...
public String obtenerSeguimientoCurricular(){
	...
	public float calcularPromedio(){
		...
		for (int i=0 ; i<this.cargaAcademica.obtenerSemestres().size() ; i++){
			Semestre s1 = this.cargaAcademica.obtenerSemestre(i) ;
			for (int j=0 ; j< s1.cursos.size();j++){
				Curso c = (Curso) s1.cursos.get(j);
				sumaNotas += c.nota ;
				cantidadCursos += 1;
			}
		}
		if (cantidadCursos!=0){
			prom = (sumaNotas / (cantidadCursos - cargaAcademica.obtenerSemestre(cargaAcademica.semestres.size()-1).cursos.size())) ;
			return prom;
		}
		else
			return 0;
